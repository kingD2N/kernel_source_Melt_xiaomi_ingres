...
/*
 * NOTE: these externs must stay here, above susfs_statfs_by_dentry() (its
 * first call site). fs/statfs.c only includes susfs_def.h, which does not
 * declare either symbol, so the upstream gki-android12-5.10 susfs patch's
 * placement of these externs after vfs_get_fsid() causes:
